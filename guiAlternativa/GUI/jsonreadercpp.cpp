...
Vector<int> *jsonReaderCpp::readPueblo(string pJson)
{
    Json::Value root;
    Json::Reader reader;
    ...
    int tmpFittest = stoi(root["Fittest"].asString());
    int tmpWorstFitness = stoi(root["worstFitness"].asString());
    int tmpAmountPeople = stoi(root["amountPeople"].asString());

    Vector<int>* puebloInfo = new Vector<int>(3);

    *(*puebloInfo)[0]= tmpFittest;
    *(*puebloInfo)[1]= tmpWorstFitness;
    *(*puebloInfo)[2]= tmpAmountPeople;
